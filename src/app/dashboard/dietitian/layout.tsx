...
export default function DietitianLayout({ children }: { children: ReactNode }) {
  ...
  if (!session || session.user.role !== "dietitian") redirect("/auth/login");
    ...
    <div className="flex min-h-screen">
      ...
      `}>
        ...
        <div className="px-6 py-7 border-b border-emerald-100/70">
          ...
          <p className="text-xs text-emerald-500/80 mt-1 font-medium">Dr. {session.user.name}</p>
        </div>
        ...
        <div className="flex flex-col gap-1 flex-1 px-3 py-4">
          {dietitianNavItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`text-base ${isActive ? "text-emerald-700" : "text-emerald-400"}`}>
                  {item.icon}
                </span>
